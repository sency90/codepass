...
void cleanHouse() {
	...
	mapUpdate();                         // 3*3 정보를 Map[][]에 기록하기

	rnt fwcnt = 0;
	while (1) {
		rnt ret = 0;
		rnt nr = row + dr[dir], nc = col + dc[dir];   // 현재방향으로 한칸 전진한 위치
		if (Map[nr][nc] == 0 || Map[nr][nc] == 3) {   // 직진 해볼만 하다면
			if (move()) {
				ret = 1;                              // 이동했으므로 결과는 1이다.
				row = nr, col = nc;                   // 위치만 이동, 방향은 변화없음
				fwcnt++; // 직진 횟수 증가
			}
		}
		if(ret == 0) {  // 직진할 수 없는 경우 방향을 회전해 보기
			if (fwcnt) {
				mapUpdate();
				fwcnt = 0;
			}
		...
		if (ret == 0) {                               // 이동이 불가능한 경우 가장 비용이 적게 드는 청소할 곳을 찾기
			...
			else fwcnt = 3;                            // 청소가능한 위치를 찾은 경우
		}

		Map[row][col] = 2;   // user.cpp의 (row, col)셀을 청소하기
